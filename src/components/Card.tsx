...
export function Card({ id, title, image, subTitle, price }: Props) {
  return (
    <Link href={`/product/${id}`} className="w-[282px] relative hover:opacity-80 transition-opacity max-h-[441px]">
      <Image src={image} alt={title} width={282} height={300} quality={100} className="rounded-xl mb-[30px] h-[370px]"/>
