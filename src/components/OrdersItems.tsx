...
export function OrdersItems({ userOrders }: Props) {
  const [toggleOrderOption, setToggleOrderOption] = useState<OrderOption>(
    OrderOption.active
  );
  const filteredData = userOrders.filter(
    (order) => order.status === toggleOrderOption
  );
  return (
    <>
      ...
      {filteredData.length >= 1 ? (
            ...
            <>
              <div className="flex flex-col gap-[30px]" key={order.id}>
                <div className="bg-white-light py-1 px-1 lg:py-7 lg:px-12 rounded-lg">
                  <p className="text-xl text-gray-text-menu font-semibold mb-[14px]">
                    Order no: #{order.id}
                  </p>
                  <div className="flex items-center justify-between">
                    <p className="flex items-center text-sm font-semibold text-gray-light gap-2">
                      Order Date:
                      <span className="text-gray-border font-thin">
                  ...
                  <div className="flex items-center justify-between">
                    <p className="flex items-center text-sm font-semibold text-gray-light gap-2">
                      ...
                      <span className="text-gray-border font-thin">
                        {format(addDays(order.createdAt, 7), "dd/MM/yyyy")}
                      </span>
                    </p>
                    <p className="flex items-center text-sm font-semibold text-gray-light gap-2">
                      Payment Method:
        ...
        <p className="text-gray-500">
          Sorry, you don&apos;t have orders on this area yet.
        </p>
      )}
    </>
  );
